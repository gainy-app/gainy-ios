...
final class TickerDetailsAboutViewCell: TickerDetailsViewCell {
    ...
    @IBAction func showMoreAction(_ sender: UIButton) {
        ...
        if sender.isSelected {
            GainyAnalytics.logEventAMP("ticker_about_more_tapped", params: ["tickerSymbol" : self.tickerInfo?.symbol ?? "none", "tickerType": self.tickerInfo?.ticker.type ?? ""])
        } else {
            GainyAnalytics.logEventAMP("ticker_about_less_tapped", params: ["tickerSymbol" : self.tickerInfo?.symbol ?? "none", "tickerType": self.tickerInfo?.ticker.type ?? ""])
        }
    }
...
class TagView: UIButton {
    ...
    func setBorderForCollection() {
        ...
        tagLabel.textColor = UIColor(hexString: "#6C5DD3", alpha: 1.0)
    }
    
    func setBorderForTicker() {
        layer.borderWidth = 1.0
        layer.borderColor = UIColor(hexString: "#E7EAEE", alpha: 1.0)?.cgColor
        tagLabel.textColor = UIColor.Gainy.mainText
    }
}
