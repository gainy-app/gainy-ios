...
extension LinkUnlinkPlaidViewController: ButtonTableViewCellDelegate {
    ...
    func connectButtonTouchUpInside(_ sender: ButtonTableViewCell) {
        ...
        Network.shared.apollo.fetch(query: ReCreatePlaidLinkQuery.init(profileId: profileID, accessTokenId: accountID, redirectUri: Constants.Plaid.redirectURI, env: UserProfileManager.shared.plaidEnv)) {[weak self] result in
            self?.hideLoader()
            switch result {
            case .success(let graphQLResult):
                guard let linkToken = graphQLResult.data?.createPlaidLinkToken?.linkToken else {
