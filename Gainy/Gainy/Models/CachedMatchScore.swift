                ...
                withCheckedContinuation { continuation in
                    Network.shared.apollo.fetch(query: GetSelectedInterestsQuery(ids: ids)){ result in
                        ...
                        case .success(let graphQLResult):
                            ...
                            continuation.resume(returning: appInterests.compactMap({TickerTag(name: "IT " + ($0.name ?? ""), url: $0.iconUrl ?? "", collectionID: -404, id: $0.id ?? -1)}))
                            break
                        case .failure(_):
                            continuation.resume(returning: [])
                            break
                ...
                withCheckedContinuation { continuation in
                    Network.shared.apollo.fetch(query: GetSelectedCategoriesQuery(ids: ids)){ result in
                        ...
                        case .success(let graphQLResult):
                            ...
                            continuation.resume(returning: categories.compactMap({TickerTag(name: "C " + ($0.name ?? ""), url: "", collectionID: -404, id: $0.id ?? -1)}))
                            break
                        case .failure(_):
                            continuation.resume(returning: [])
                            break
