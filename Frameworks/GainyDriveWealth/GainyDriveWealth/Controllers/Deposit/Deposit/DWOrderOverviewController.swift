            ...
            do {
                ...
                await MainActor.run {
                    if mode == .sell {
                        ...
                        GainyAnalytics.logEvent("dw_stock_sell_done", params: ["amount" : amount, "symbol" : symbol])
                    } else {
                        coordinator?.showOrderSpaceDone(amount: amount, collectionId: collectionId, name: name, type: type)
                        if mode == .buy {
                            GainyAnalytics.logEvent("dw_stock_buy_done", params: ["amount" : amount, "symbol" : symbol])
                        } else {
                            GainyAnalytics.logEvent("dw_stock_invest_done", params: ["amount" : amount, "symbol" : symbol])
                        }
                    }
